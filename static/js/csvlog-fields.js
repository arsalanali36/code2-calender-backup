...
function _renderDayContent(body) {
  ...
  let totalPnl = 0, totalPts = 0, totalFee = 0, pnlValid = true, ptsValid = true;
  const allRows = _clDayTrades.map(({ trade }, i) => {
    ...
    if (!isNaN(pnlNum)) totalPnl += pnlNum; else pnlValid = false;
    if (!isNaN(ptsNum)) totalPts += ptsNum; else ptsValid = false;
  ...
  const pnlColor = pnlValid ? (totalPnl > 0 ? 'var(--green)' : totalPnl < 0 ? 'var(--red)' : '') : '';
  const ptsColor = ptsValid ? (totalPts > 0 ? 'var(--green)' : totalPts < 0 ? 'var(--red)' : '') : '';
  stats.appendChild(mkStat('Total P/L (Rs)', pnlValid ? fmtNum(totalPnl) : '—', pnlColor));
  stats.appendChild(mkStat('Total Points',   ptsValid ? fmtNum(totalPts) : '—', ptsColor));
  ...
  // ── P/L bar chart ──────────────────────────────────────────────────────────
  const chartRows = rows.filter(r => r.pnlNum !== null);
  ...
  thead.innerHTML = '<tr><th>#</th><th>Instrument</th><th>Entry</th><th>Exit</th><th>Dur</th><th>P/L (Rs)</th><th>Pts</th></tr>';
