...
import { Heart, MessageCircle, Send, X, Lock, Unlock, Calendar } from 'lucide-react';
interface DayData {
  date: string;
  images: string[];
  tradeNum?: number;
...
export const FullscreenViewer: React.FC<FullscreenViewerProps> = ({ days, initialDayIndex, initialImageIndex, isOpen, onClose, initialLocked = false }) => {
  ...
  const [uiVisible, setUiVisible] = useState(false); // Default to Hidden for clean view
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      // Restore state for the initial image if it exists in cache
  ...
  // Stable Ref for scale to use in event listeners without re-adding them
  const scaleRef = React.useRef(scale);
  React.useEffect(() => { scaleRef.current = scale; }, [scale]);

  // Pinch-to-zoom handler - Added once
  React.useEffect(() => {
  ...
  const restoreState = (dIdx: number, iIdx: number) => {
    ...
    if (cached) {
      ...
      setPan({ x: 0, y: 0 }); // keep current scale as a global preference but reset pan
    }
  };

  // Move to next/prev item (trade) regardless of date — used at image boundaries
  const nextItem = () => {
    if (dayIdx < days.length - 1) {
      saveCurrentState();
      restoreState(dayIdx + 1, 0);
      setDayIdx(dayIdx + 1);
      setImgIdx(0);
    }
  };
  ...
  const nextImg = () => {
    if (imgIdx < images.length - 1) {
      saveCurrentState();
      setImgIdx(prev => { restoreState(dayIdx, prev + 1); return prev + 1; });
    } else {
      nextItem();
    }
  };
  ...
  const nextDay = () => {
    // Jump to first item of the next date
  ...
  const jumpToDate = (selectedDate: string) => {
    ...
    if (foundIdx !== -1) {
      saveCurrentState();
      restoreState(foundIdx, 0);
      setDayIdx(foundIdx);
      setImgIdx(0);
    } else {
      console.log("No trade found for date:", selectedDate);
    }
  };
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        ref={containerRef}
      >
        {/* Header - Simple clean fade */}
        <motion.div 
          animate={{ y: (uiVisible || isLocked) ? 0 : -20, opacity: (uiVisible || isLocked) ? 1 : 0 }}
          transition={{ duration: 0.2, ease: "linear" }}
          className="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-black/80 to-transparent z-[110] flex items-center justify-between px-4"
        >
          ...
          <div className="flex flex-col items-center">
            <span 
              className="relative text-[11px] font-bold uppercase tracking-widest flex items-center gap-1.5 cursor-pointer hover:bg-white/10 active:scale-95 transition-all px-2.5 py-1.5 rounded-lg border border-white/5 shadow-sm"
            >
              <Calendar className="w-3.5 h-3.5 text-indigo-400" />
              <span className="text-white">{formatDateLabel(currentDay.date)}</span>
              <span className="text-indigo-500">•</span>
              <span className="text-white">T{(currentDay as any).tradeNum || (dayIdx + 1)}</span>
              <span className="text-indigo-500">•</span>
              <span className="text-white">{imgIdx + 1}/{images.length}</span>
              <input 
                type="date"
                ref={dateInputRef}
          ...
          <button
            onClick={(e) => {
              ...
              if (newLocked) {
                // Must enter fullscreen first, then lock orientation
        ...
        <motion.div 
          ...
          onDragEnd={(e, info) => {
            ...
            if (Math.abs(tx) > Math.abs(ty)) {
              if (tx > 50) prevImg();
              else if (tx < -50) nextImg();
            } else {
              if (ty > 60) prevDay(); 
              else if (ty < -60) nextDay();
            }
          ...
          onClick={(e) => {
            const now = Date.now();
            const delay = 300;
            if ((window as any)._lastTap && (now - (window as any)._lastTap < delay)) {
        ...
        >
          <motion.img
            ...
            transition={{ duration: 0, ease: 'linear' }} // Zero duration for instant feel during navigation/pan
            drag={scale > 1}
            dragMomentum={false}
            onDrag={(e, info) => {
              if (scale > 1) {
                // Calculate boundaries based on scale
                const boundaryX = (scale - 1) * 150; 
                const boundaryY = (scale - 1) * 250;
                
                setPan({ 
                  x: Math.max(-boundaryX, Math.min(boundaryX, pan.x + info.delta.x)), 
                  y: Math.max(-boundaryY, Math.min(boundaryY, pan.y + info.delta.y)) 
                });
              }
            }}
          ...
          >
            {images.map((_, i) => (
              <div 
                key={i} 
                className={`h-1 rounded-full transition-all duration-300 ${i === imgIdx ? 'w-6 bg-white' : 'w-2 bg-white/30'}`} 
              />
            ))}
          </motion.div>
        ...
        {/* Navigation buttons — OUTSIDE draggable div, fixed to container */}
        <AnimatePresence>
          {isLocked && (
            <>
              <motion.button
              ...
              <motion.button
                ...
                className="absolute bottom-5 left-5 w-[56px] h-[56px] rounded-full bg-black/55 backdrop-blur-md border border-white/25 flex items-center justify-center active:scale-90 transition-transform shadow-xl z-[140]"
              >
              ...
              <motion.button
                ...
                className="absolute bottom-5 right-5 w-[56px] h-[56px] rounded-full bg-black/55 backdrop-blur-md border border-white/25 flex items-center justify-center active:scale-90 transition-transform shadow-xl z-[140]"
              >
                <span className="text-2xl font-bold text-white">↓</span>
              </motion.button>
              ...
              <motion.div
                ...
                className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 z-[140] w-[60%] pointer-events-none"
              >
                ...
                <span className="text-[10px] font-black text-white bg-black/50 px-3 py-1 rounded-full whitespace-nowrap">{scale.toFixed(1)}x</span>
              </motion.div>
            </>
          )}
        </AnimatePresence>

        {/* Sidebar Actions - Simple fade */}
        <motion.div 
          animate={{ x: (uiVisible && !isLocked) ? 0 : 20, opacity: (uiVisible && !isLocked) ? 1 : 0 }}
          transition={{ duration: 0.2, ease: "linear" }}
          className="absolute right-2 bottom-32 flex flex-col items-center gap-6 z-20"
        >
          <div className="flex flex-col items-center">
        ...
        {/* Bottom Info - Simple fade */}
        <motion.div 
          animate={{ y: (uiVisible && !isLocked) ? 0 : 20, opacity: (uiVisible && !isLocked) ? 1 : 0 }}
          transition={{ duration: 0.2, ease: "linear" }}
          className="p-6 bg-gradient-to-t from-black/90 to-transparent z-10"
        >
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center font-bold text-xs">TJ</div>
          ...
          <div className="bg-white/10 rounded-full px-4 py-2 flex items-center gap-3 border border-white/10">
